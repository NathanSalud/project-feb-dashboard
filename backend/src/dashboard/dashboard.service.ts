...
export class DashboardService {
  ...
  async getKpis(companyName: string, isAdmin: boolean) {
    const whereClause = isAdmin
      ? `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's'`
      : `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's' AND a.COMPANY_NAME = '${companyName}'`;
  ...
  async getTimeSeries(companyName: string, isAdmin: boolean) {
    const whereClause = isAdmin
      ? `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's'`
      : `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's' AND a.COMPANY_NAME = '${companyName}'`;
  ...
  async getShops(companyName: string, isAdmin: boolean) {
    const whereClause = isAdmin
      ? `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's'`
      : `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's' AND a.COMPANY_NAME = '${companyName}'`;
    ...
    return this.snowflake.query(`
      SELECT
        ...
        COUNT(f.ORDER_ITEM_SK)                        AS ITEMS,
        ROUND(SUM(f.ORIGINAL_PRODUCT_PRICE), 2)       AS REVENUE,
        ROUND(SUM(f.ORIGINAL_PRODUCT_PRICE) /
              NULLIF(COUNT(DISTINCT f.PLATFORM_ORDER_ID), 0), 2) AS AOV
      FROM GDEC_DATAMART.GOLD_SCHEMA.FACT_PLATFORM_ORDER_ITEMS f
      INNER JOIN GDEC_DATAMART.GOLD_SCHEMA.DIM_MARKETPLACE_ACCOUNTS a
        ON f.SHOP_ID = a.SHOP_ID
      WHERE f.ITEM_STATUS IN ('Completed','Shipped','Delivered','Ready to Ship')
  ...
  async getProducts(companyName: string, isAdmin: boolean) {
    const whereClause = isAdmin
      ? `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's'`
      : `a.IS_ACTIVE = TRUE AND a.ACCOUNT_NAME != 's' AND a.COMPANY_NAME = '${companyName}'`;
